...
class Crazyflie:
    ...
    def getTarget(self):
        ...
        if(self.error(target_yaw[0],self.yaw)<0.01):
            print("true")
        if(self.error(self.target[0],self.xGPS)<0.007 and self.error(self.target[1],self.yGPS)<0.007 and self.error(self.target[2],self.zGPS)<0.007 and self.error(target_yaw[0],self.yaw)<0.007):
